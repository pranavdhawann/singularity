...
const redactionPatterns: Array<{ kind: RedactionKind; pattern: RegExp; replacement: string }> = [
  { kind: "private_key", pattern: /-----BEGIN [A-Z ]*PRIVATE KEY-----[\s\S]+?-----END [A-Z ]*PRIVATE KEY-----/g, replacement: "[REDACTED_PRIVATE_KEY]" },
  ...
  { kind: "phone", pattern: /\b(?:\+?1[-.\s]?)?\(?\d{3}\)?[-.\s]?\d{3}[-.\s]?\d{4}\b/g, replacement: "[REDACTED_PHONE]" },
  { kind: "credential_path", pattern: /\b[A-Z]:\\[^\s]*(?:\.ssh|credentials|secrets|tokens)[^\s]*/gi, replacement: "[REDACTED_PATH]" }
];
