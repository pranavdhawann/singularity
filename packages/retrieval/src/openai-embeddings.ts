...
interface OpenAiEmbeddingRow { index?: unknown; embedding?: unknown }
...
export class OpenAiCompatibleEmbeddingAdapter implements EmbeddingAdapter {
  ...
  async embed(input: EmbeddingInput): Promise<EmbeddingResult> {
    ...
    try {
      ...
      return { available: true, vectors: validateVectors(ordered.map((row) => row.embedding), input.texts.length) };
    } catch (error) {
