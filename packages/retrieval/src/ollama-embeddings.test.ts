...
describe("OllamaEmbeddingAdapter", () => {
  ...
  it("returns safe typed errors for invalid provider responses", async () => {
    ...
    await expect(
      new OllamaEmbeddingAdapter({ baseUrl: "http://localhost:11434", fetch }).embed({ model: "embed", texts: ["a"] }),
    ).rejects.toEqual(expect.any(EmbeddingAdapterError));
    try {
      await new OllamaEmbeddingAdapter({ baseUrl: "http://localhost:11434", fetch }).embed({
        model: "embed",
        texts: ["a"],
      });
    } catch (error) {
