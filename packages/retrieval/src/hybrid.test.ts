...
function candidate(
  overrides: Partial<HybridRetrievalCandidate> & Pick<HybridRetrievalCandidate, "id" | "kind">,
...
describe("rankHybridCandidates", () => {
  it("supports lexical-only and fuses vectors with explicit reasons", () => {
    const lexical = rankHybridCandidates({
      workspaceId: "w_1",
      candidates: [
  ...
  it("filters unauthorized and compacted sources and boosts pinned memories", () => {
    const results = rankHybridCandidates({
      workspaceId: "w_1",
      suppressedSourceKeys: ["timeline_event:old:hash_old"],
      candidates: [
