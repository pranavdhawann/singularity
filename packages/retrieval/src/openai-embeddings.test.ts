...
describe("OpenAiCompatibleEmbeddingAdapter", () => {
  it("resolves the secret at call time and normalizes indexed vectors", async () => {
    ...
    const adapter = new OpenAiCompatibleEmbeddingAdapter({
      baseUrl: "https://models.example/v1/", apiKeyRef: "FUTURE_KEY", resolveSecret: secret, fetch
    });
    ...
    expect(fetch).toHaveBeenCalledWith("https://models.example/v1/embeddings", expect.objectContaining({
  ...
  it("rejects count and dimension mismatches without leaking secrets", async () => {
    const fetch = vi.fn(async () => new Response(JSON.stringify({ data: [{ index: 0, embedding: [1] }] }), { status: 200 }));
    const adapter = new OpenAiCompatibleEmbeddingAdapter({ baseUrl: "https://models.example/v1",
      apiKeyRef: "KEY", resolveSecret: () => "never-leak", fetch });
    await expect(adapter.embed({ model: "embed", texts: ["a", "b"] })).rejects.toEqual(expect.any(EmbeddingAdapterError));
    try { await adapter.embed({ model: "embed", texts: ["a", "b"] }); } catch (error) {
  ...
  it("propagates aborts", async () => {
    ...
    const adapter = new OpenAiCompatibleEmbeddingAdapter({ baseUrl: "https://models.example/v1",
      apiKeyRef: "KEY", resolveSecret: () => "secret", fetch });
