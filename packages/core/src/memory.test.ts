...
describe("Phase 3 memory contracts", () => {
  it("represent versioned, namespaced memory with provenance", () => {
    const memory = {
      ...
      pinned: true,
      version: 2,
      namespaceIds: ["ns_code"],
      primaryNamespaceId: "ns_code",
      sourceIds: ["evt_1"],
  ...
  it("represents namespaces, revisions, and source-linked compactions", () => {
    ...
    const revision = {
      ...
      next: { statement: "Use SQLite for local state" },
      reason: "user_edit",
    ...
    const compaction = {
      id: "cmp_1",
      workspaceId: "w_1",
      summary: "SQLite is the local source of truth.",
      contentHash: "hash_1",
      sources: [{ kind: "memory", id: "mem_1", contentHash: "hash_mem" }],
      invalidatedAt: null,
