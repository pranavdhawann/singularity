...
export class ImportJobRepository {
  ...
  createFile(input: CreateImportFileInput): ImportJobDto {
    ...
    const create = this.db.transaction(() => {
      ...
      this.db.prepare(
        ...
        inputJson: JSON.stringify({ importId, filename: input.filename, mediaType: input.mediaType, kind: input.kind }),
        now
      });
      this.db.prepare(
      ...
      this.db.prepare("INSERT INTO import_payloads (import_id, content) VALUES (?, ?)")
        .run(importId, content);
    });
    create();
  ...
  get(jobId: string): ImportJobDto | undefined {
    const row = this.db.prepare<{ jobId: string }, ImportJobRow>(
  ...
  listForWorkspace(workspaceId: string): ImportJobDto[] {
    return this.db.prepare<{ workspaceId: string }, ImportJobRow>(
      `SELECT j.id, i.id AS import_id, i.workspace_id, i.filename, i.media_type,
        i.byte_size, j.status, c.document_index, c.next_chunk_index,
        i.document_count, i.completed_document_count, j.error_message,
        j.created_at, i.updated_at
    ...
    ).all({ workspaceId }).map(mapImportJob);
  }

  readPayload(importId: string): Buffer {
    const row = this.db.prepare<{ importId: string }, { content: Buffer }>(
  ...
  fail(jobId: string, errorCode: string): ImportJobDto {
    ...
    const fail = this.db.transaction(() => {
      this.db.prepare(
        "UPDATE jobs SET status = 'failed', error_message = @errorCode, finished_at = @now WHERE id = @jobId"
      ).run({ jobId, errorCode, now });
      this.db.prepare(
        "UPDATE import_job_checkpoints SET phase = 'failed', updated_at = @now WHERE job_id = @jobId"
      ).run({ jobId, now });
      this.db.prepare(
  ...
  retry(jobId: string): ImportJobDto {
    ...
    const retry = this.db.transaction(() => {
      ...
      this.db.prepare(
        "UPDATE import_job_checkpoints SET phase = 'queued', updated_at = @now WHERE job_id = @jobId"
      ).run({ jobId, now });
      this.db.prepare(
