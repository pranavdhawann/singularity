...
describe("SearchRepository", () => {
  it("searches active workspace documents, memories, and text events", () => {
    ...
    try {
      db.client
        .prepare(`INSERT INTO documents VALUES (?, ?, NULL, ?, ?, ?, ?, NULL, ?)`)
        .run("doc_1", "w_1", "Architecture", "memory://doc_1", "text/plain", "doc_hash", "2026-07-11T00:00:00.000Z");
      db.client
        .prepare(
        ...
        .run("chunk_1", "doc_1", 0, "SQLite stores local documents", 4, "2026-07-11T00:00:00.000Z", "chunk_hash_1");
      db.client
        .prepare(`INSERT INTO document_chunks_fts VALUES (?, ?, ?)`)
        .run("chunk_1", "Architecture", "SQLite stores local documents");
      const memories = new MemoryRepository(db.client);
      memories.create({
        workspaceId: "w_1",
        type: "decision",
        statement: "SQLite is the source of truth",
        confidence: 0.9,
      ...
      events.append(
        createEvent({
          ...
          actor: "user",
          title: "Message",
          payload: { text: "Remember the SQLite decision" },
  ...
  it("returns empty for punctuation and stable ordering for ties", () => {
    ...
    try {
      ...
      expect(search.search({ workspaceId: "w_1", query: "same" }).map((item) => item.id)).toEqual(
        search.search({ workspaceId: "w_1", query: "same" }).map((item) => item.id),
      );
    } finally {
      db.close();
    }
