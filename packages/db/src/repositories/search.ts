...
interface RankedCandidate extends Omit<UnifiedSearchCandidate, "lexicalScore"> {
  rawScore: number;
}
...
export class SearchRepository {
  ...
  listAuthorized(workspaceId: string, limit = 200): UnifiedSearchCandidate[] {
    ...
    const documents = this.db
      .prepare<{ workspaceId: string }, DocumentRow>(
      ...
      .all({ workspaceId })
      .map((row): UnifiedSearchCandidate => ({
        kind: "document_chunk",
        id: row.id,
        workspaceId,
        title: row.title,
        ...
        contentHash: hash(row.text),
        lexicalScore: 0,
        ...(row.source_range_json
          ? { sourceRange: JSON.parse(row.source_range_json) as { start: number; end: number } }
          : {}),
      }));
    const memories = this.db
      .prepare<{ workspaceId: string }, MemoryRow>(
      ...
      .map((row): UnifiedSearchCandidate => ({
        ...
        id: row.id,
        workspaceId,
        title: row.pinned ? "Pinned memory" : "Approved memory",
        text: row.statement,
    ...
    const compactions = this.db
      .prepare<{ workspaceId: string }, CompactionRow>(
      ...
      .map((row): UnifiedSearchCandidate => ({
        ...
        text: row.summary,
        tokenCount: estimateTokens(row.summary),
        contentHash: row.content_hash || hash(row.summary),
        lexicalScore: 0,
        createdAt: row.created_at,
      }));
  ...
  private searchDocuments(workspaceId: string, query: string): RankedCandidate[] {
    ...
    return this.db
      .prepare<{ workspaceId: string; query: string }, Row>(
        `SELECT dc.id, d.title, dc.text, dc.token_count, dc.source_range_json,
