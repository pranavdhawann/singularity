...
export class MemoryRepository {
  ...
  list(input: MemoryListInput): { items: MemoryDto[]; nextCursor?: string } {
    ...
    if (input.reviewState) {
      where.push("m.review_state = @reviewState");
    ...
    const rows = this.db
      .prepare<Record<string, string | number>, MemoryRow>(
  ...
  mutate(id: string, input: MemoryMutationInput, appendEvent: (memory: MemoryDto) => void = () => {}): MemoryDto {
    return this.db.transaction(() => {
      ...
      if (current.version !== input.expectedVersion)
        throw new MemoryConflictError(input.expectedVersion, current.version);
  ...
  delete(id: string, expectedVersion: number, appendEvent: (memory: MemoryDto) => void = () => {}): MemoryDto {
    return this.db.transaction(() => {
      ...
      const now = new Date().toISOString();
      this.db
        .prepare(`UPDATE memories SET deleted_at = @now, updated_at = @now, version = version + 1 WHERE id = @id`)
        .run({ id, now });
      const next = this.get(id, { includeDeleted: true })!;
  ...
  listRevisions(memoryId: string): MemoryRevisionDto[] {
    return this.db
      .prepare<{ memoryId: string }, RevisionRow>(
      ...
      .map((row) => {
        ...
        return {
          ...
          previous: JSON.parse(row.previous_json) as Record<string, unknown>,
          next,
          reason: row.reason,
  ...
  private map(row: MemoryRow): MemoryDto {
    const memberships = this.db
      .prepare<{ id: string }, MembershipRow>(
    ...
    return {
      ...
      pinned: row.pinned === 1,
      version: row.version,
      namespaceIds: memberships.map((membership) => membership.namespace_id),
      ...(primary ? { primaryNamespaceId: primary } : {}),
      sourceIds,
  ...
  private validateNamespaces(workspaceId: string, namespaceIds?: string[], primary?: string | null): void {
    ...
    for (const id of new Set(namespaceIds)) {
      if (lookup.get({ id })?.workspace_id !== workspaceId)
        throw new Error("namespace must belong to memory workspace");
    }
  }
