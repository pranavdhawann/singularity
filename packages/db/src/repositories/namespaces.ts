...
export class NamespaceRepository {
  ...
  create(input: CreateNamespaceInput): MemoryNamespaceDto {
    ...
    const duplicate = this.db
      .prepare<{ workspaceId: string; parentId: string | null; name: string }, { id: string }>(
  ...
  get(id: string): MemoryNamespaceDto | undefined {
    return mapNamespace(
      this.db.prepare<{ id: string }, NamespaceRow>("SELECT * FROM memory_namespaces WHERE id = @id").get({ id }),
    );
  }

  list(workspaceId: string): MemoryNamespaceDto[] {
    return this.db
      .prepare<{ workspaceId: string }, NamespaceRow>(
