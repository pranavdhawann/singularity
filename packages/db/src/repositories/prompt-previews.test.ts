...
const preview: PromptPreviewDto = {
  ...
  endpointClassification: "external", contextPackId: "pack_1",
  contextPackHash: "pack-hash", redactedPrompt: "safe prompt",
  promptHash: "prompt-hash", bindingHash: "binding-hash", estimatedTokens: 3,
  privacyLabels: ["private"], redactionCounts: { secret: 1 },
...
describe("PromptPreviewRepository", () => {
  it("stores an immutable preview and one immutable decision", () => {
    ...
    try {
      ...
      const decision = repository.decide({
        id: "decision_1", previewId: preview.id, decision: "approved",
        bindingHash: preview.bindingHash, decidedAt: "2026-07-11T00:01:00.000Z"
      });

      expect(decision.decision).toBe("approved");
      expect(() => repository.decide({
        id: "decision_2", previewId: preview.id, decision: "denied",
        bindingHash: preview.bindingHash, decidedAt: "2026-07-11T00:02:00.000Z"
  ...
  it("rejects mismatched and expired decisions", () => {
    ...
    try {
      ...
      expect(() => repository.decide({
        id: "decision_1", previewId: preview.id, decision: "approved",
        bindingHash: "changed", decidedAt: "2026-07-11T00:01:00.000Z"
      })).toThrow(PromptPreviewConflictError);
      expect(() => repository.decide({
        id: "decision_2", previewId: preview.id, decision: "approved",
        bindingHash: preview.bindingHash, decidedAt: "2026-07-11T00:06:00.000Z"
