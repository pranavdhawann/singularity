...
import {
  ...
  type MemoryCompactionSourceDto,
} from "@future/core";
...
interface SourceRow {
  source_kind: "memory" | "timeline_event";
  source_id: string;
  content_hash: string;
}
...
export class CompactionRepository {
  ...
  get(id: string): MemoryCompactionDto | undefined {
    const row = this.db.prepare<{ id: string }, CompactionRow>("SELECT * FROM compactions WHERE id = @id").get({ id });
    ...
    const sources = this.db
      ...
      .all({ id })
      .map((source) => ({ kind: source.source_kind, id: source.source_id, contentHash: source.content_hash }));
    return {
      id: row.id,
  ...
  activeSourceKeys(workspaceId: string): string[] {
    return this.db
      .prepare<{ workspaceId: string }, SourceRow>(
      ...
      .all({ workspaceId })
      .map((source) => `${source.source_kind}:${source.source_id}:${source.content_hash}`);
  }
  ...
  private validateSources(workspaceId: string, sources: readonly MemoryCompactionSourceDto[]): void {
    for (const source of sources) {
      const row =
        source.kind === "memory"
          ? this.db
              .prepare<{ id: string }, { workspace_id: string; content_hash: string }>(
  ...
  private eventSource(id: string): { workspace_id: string; content_hash: string } | undefined {
    const row = this.db
      .prepare<{ id: string }, { workspace_id: string; payload_json: string }>(
        "SELECT workspace_id, payload_json FROM events WHERE id = @id",
      )
      .get({ id });
    return row
      ? { workspace_id: row.workspace_id, content_hash: createHash("sha256").update(row.payload_json).digest("hex") }
      : undefined;
  }
}
