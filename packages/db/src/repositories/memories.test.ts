...
describe("MemoryRepository", () => {
  it("creates, filters, revises, and tombstones namespaced memory", () => {
    ...
    try {
      ...
      const revised = memories.mutate(created.id, {
        ...
        pinned: true,
        namespaceIds: [coding.id, future.id],
        primaryNamespaceId: future.id,
        reason: "user_edit",
      });

      expect(revised).toMatchObject({ version: 2, pinned: true, primaryNamespaceId: future.id });
      expect(memories.list({ workspaceId: "w_1", reviewState: "approved", namespaceId: coding.id }).items).toEqual([
        revised,
      ]);
  ...
  it("uses stable cursors and rejects stale versions", () => {
    ...
    try {
      ...
      expect(() => memories.mutate(first.id, { expectedVersion: 9, pinned: true, reason: "stale" })).toThrow(
        MemoryConflictError,
      );
    } finally {
      db.close();
