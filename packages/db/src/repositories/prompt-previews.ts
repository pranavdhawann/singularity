...
interface PromptPreviewRow {
  id: string; workspace_id: string; turn_id: string; provider_id: string;
  model_profile_id: string; model: string; endpoint_classification: "external";
  context_pack_id: string; context_pack_hash: string; redacted_prompt: string;
  prompt_hash: string; binding_hash: string; estimated_tokens: number;
  privacy_labels_json: string; redaction_counts_json: string;
  selected_sources_json: string; excluded_sources_json: string;
  created_at: string; expires_at: string; invalidated_at: string | null;
}

interface PromptDecisionRow {
  id: string; preview_id: string; decision: "approved" | "denied";
  binding_hash: string; decided_at: string;
}

export class PromptPreviewConflictError extends Error {
  constructor() { super("prompt preview conflict"); this.name = "PromptPreviewConflictError"; }
}

export class PromptPreviewExpiredError extends Error {
  constructor() { super("prompt preview expired"); this.name = "PromptPreviewExpiredError"; }
}
...
export class PromptPreviewRepository {
  ...
  get(id: string): PromptPreviewDto | undefined {
    const row = this.db.prepare<{ id: string }, PromptPreviewRow>(
      "SELECT * FROM prompt_previews WHERE id = @id"
    ).get({ id });
    return row ? mapPreview(row) : undefined;
  }

  getForWorkspace(id: string, workspaceId: string): PromptPreviewDto | undefined {
    const row = this.db.prepare<{ id: string; workspaceId: string }, PromptPreviewRow>(
  ...
  decide(decision: PromptDecisionDto): PromptDecisionDto {
    const save = this.db.transaction(() => {
      ...
      if (!row || row.invalidated_at || row.binding_hash !== decision.bindingHash || this.getDecision(decision.previewId)) {
  ...
  isInvalidated(id: string): boolean {
    return this.db.prepare<{ id: string }, number>(
...
function mapPreview(row: PromptPreviewRow): PromptPreviewDto {
  return {
    id: row.id, workspaceId: row.workspace_id, turnId: row.turn_id,
    providerId: row.provider_id, modelProfileId: row.model_profile_id,
    model: row.model, endpointClassification: row.endpoint_classification,
    contextPackId: row.context_pack_id, contextPackHash: row.context_pack_hash,
    redactedPrompt: row.redacted_prompt, promptHash: row.prompt_hash,
    bindingHash: row.binding_hash, estimatedTokens: row.estimated_tokens,
...
function mapDecision(row: PromptDecisionRow): PromptDecisionDto {
  return {
    id: row.id, previewId: row.preview_id, decision: row.decision,
    bindingHash: row.binding_hash, decidedAt: row.decided_at
  };
}
