...
export interface EmbeddingRecord {
  id: string; workspaceId: string; sourceKind: string; sourceId: string; contentHash: string;
  adapter: string; model: string; dimensions: number; vector: number[]; createdAt: string;
}
export interface UpsertEmbeddingInput extends Omit<EmbeddingRecord, "id" | "dimensions" | "createdAt"> {}
export interface EmbeddingSourceKey { kind: string; id: string; contentHash: string }

interface EmbeddingRow {
  id: string; workspace_id: string; source_kind: string; source_id: string; content_hash: string;
  adapter: string; model: string; dimensions: number; vector_json: string; created_at: string;
}

export class EmbeddingDimensionError extends Error {
  constructor() { super("embedding dimensions are invalid or inconsistent"); this.name = "EmbeddingDimensionError"; }
}
...
export class EmbeddingRepository {
  ...
  upsert(input: UpsertEmbeddingInput): EmbeddingRecord {
    ...
    const expected = this.db.prepare<{ adapter: string; model: string }, { dimensions: number }>(
    ...
    return this.find(input.workspaceId, input.sourceKind, input.sourceId, input.contentHash, input.adapter, input.model)!;
  }

  listForSources(input: { workspaceId: string; adapter: string; model: string; sources: readonly EmbeddingSourceKey[] }): EmbeddingRecord[] {
    return input.sources.flatMap((source) => {
      const found = this.find(input.workspaceId, source.kind, source.id, source.contentHash, input.adapter, input.model);
      return found ? [found] : [];
    });
  }
  ...
  private find(workspaceId: string, sourceKind: string, sourceId: string, contentHash: string, adapter: string, model: string): EmbeddingRecord | undefined {
    const row = this.db.prepare<Record<string, string>, EmbeddingRow>(
...
function mapEmbedding(row: EmbeddingRow): EmbeddingRecord {
  return { id: row.id, workspaceId: row.workspace_id, sourceKind: row.source_kind,
    sourceId: row.source_id, contentHash: row.content_hash, adapter: row.adapter,
    model: row.model, dimensions: row.dimensions, vector: JSON.parse(row.vector_json) as number[],
