...
test("browser imports, resumes indexing, approves an exact external prompt, and inspects citations", async ({ page }) => {
  ...
  await page.getByLabel("Choose import files").setInputFiles([
  ...
  const events = (await timeline.json() as { events: Array<{ type: string; payload: Record<string, unknown> }> }).events;
  ...
  expect(required?.payload).toEqual(expect.objectContaining({ previewId: expect.any(String), bindingHash: expect.any(String) }));
  ...
  expect((await storedPreview.json() as { redactedPrompt: string }).redactedPrompt).not.toContain("phase4-owner@example.com");
});
