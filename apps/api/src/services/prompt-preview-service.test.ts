...
const input = {
  ...
  contextPackHash: "pack-hash", instructions: "Be useful",
...
describe("PromptPreviewService", () => {
  it("creates a redacted preview and requires its exact approved binding", () => {
    ...
    try {
      ...
      expect(service.requireGrant(preview.id, {
        turnId: preview.turnId, providerId: preview.providerId,
        modelProfileId: preview.modelProfileId, model: preview.model,
        contextPackId: preview.contextPackId, contextPackHash: preview.contextPackHash,
  ...
  it("rejects denial and changed execution inputs", () => {
    ...
    try {
      ...
      expect(() => service.requireGrant(denied.id, {
        turnId: denied.turnId, providerId: denied.providerId,
        modelProfileId: denied.modelProfileId, model: denied.model,
        contextPackId: denied.contextPackId, contextPackHash: denied.contextPackHash,
      ...
      expect(() => service.requireGrant(changed.id, {
        turnId: changed.turnId, providerId: changed.providerId,
        modelProfileId: changed.modelProfileId, model: "changed-model",
        contextPackId: changed.contextPackId, contextPackHash: changed.contextPackHash,
