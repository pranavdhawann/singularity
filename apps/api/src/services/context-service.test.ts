  ...
  it("persists approved memory, document, and prior event context for one workspace", async () => {
    ...
    try {
      ...
      insertMemory.run({
        id: "mem_approved",
        workspaceId: "w_demo",
        statement: "SQLite is the local decision.",
        confidence: 0.95,
      ...
      insertMemory.run({
        id: "mem_proposed",
        workspaceId: "w_demo",
        statement: "SQLite proposed local decision.",
        confidence: 0.99,
      ...
      insertMemory.run({
        id: "mem_other",
        workspaceId: "w_other",
        statement: "SQLite other local decision.",
        confidence: 0.99,
        reviewState: "approved",
        createdAt: now,
        updatedAt: now,
  ...
  it("uses optional vectors, persists explanations, and falls back safely", async () => {
    ...
    try {
      ...
      memories.create({
        workspaceId: "w_demo",
        type: "fact",
        statement: "shared ordinary fact",
        confidence: 1,
        reviewState: "approved",
        sourceIds: [],
      });
      memories.create({
        workspaceId: "w_demo",
        type: "fact",
        statement: "semantic preferred answer",
        confidence: 1,
        reviewState: "approved",
        sourceIds: [],
      });
      ...
      const adapter: EmbeddingAdapter = {
        ...
        async embed(input) {
          return {
            available: true,
            vectors: input.texts.map((text, index) => (index === 0 || text.includes("preferred") ? [1, 0] : [0, 1])),
          };
        },
      };
      const profile = {
      ...
      const hybrid = await service.buildForTurn({
        turnId: "t1",
        workspaceId: "w_demo",
        userEventId: "current",
        query: "shared fact",
      ...
      const fallback = await failing.buildForTurn({
        turnId: "t2",
        workspaceId: "w_demo",
        userEventId: "current",
        query: "shared fact",
  ...
  it("retrieves an active compaction while suppressing its represented memory", async () => {
    ...
    try {
      ...
      const memory = memories.create({
        workspaceId: "w_demo",
        type: "summary",
        statement: "SQLite architecture decision",
        confidence: 1,
        reviewState: "approved",
        sourceIds: [],
      });
      const hash = db.client
        .prepare<{ id: string }, { content_hash: string }>("SELECT content_hash FROM memories WHERE id = @id")
        .get({ id: memory.id })!.content_hash;
      const compactions = new CompactionRepository(db.client);
      const compaction = compactions.create({
        workspaceId: "w_demo",
        summary: "SQLite remains the architecture source of truth",
        sources: [{ kind: "memory", id: memory.id, contentHash: hash }],
      });
      const packs = new ContextPackRepository(db.client);
      const pack = await new ContextService({
        db: db.client,
        events: new EventRepository(db.client),
        contextPacks: packs,
        compactions,
      }).buildForTurn({
        turnId: "t1",
