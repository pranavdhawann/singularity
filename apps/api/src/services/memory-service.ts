...
export class MemoryService {
  ...
  create(input: CreateMemoryRecordInput): MemoryDto {
    return this.dependencies.memories.create(input, (memory) => {
      this.append(
        memory.workspaceId,
        memory.reviewState === "approved" ? "memory.approved" : "memory.proposed",
        memory.reviewState === "approved" ? "Memory approved" : "Memory proposed",
  ...
  mutate(id: string, input: MemoryMutationInput): MemoryDto {
    ...
    try {
      return this.dependencies.memories.mutate(id, input, (next) => {
        this.dependencies.embeddings.invalidateSource(
          next.workspaceId,
          "memory",
          next.id,
          hashFromDb(this.dependencies.db, next.id),
        );
        this.dependencies.compactions.invalidateForSource("memory", next.id);
        if (next.statement !== current.statement)
          this.append(next.workspaceId, "memory.revised", "Memory revised", payload(next));
        if (next.reviewState !== current.reviewState) {
          const type =
            next.reviewState === "approved"
              ? "memory.approved"
              : next.reviewState === "outdated"
                ? "memory.outdated"
                : `memory.${next.reviewState}`;
          this.append(next.workspaceId, type, `Memory ${next.reviewState}`, payload(next));
        }
        if (next.pinned !== current.pinned)
          this.append(
            next.workspaceId,
            next.pinned ? "memory.pinned" : "memory.unpinned",
            next.pinned ? "Memory pinned" : "Memory unpinned",
            payload(next),
          );
        if (input.namespaceIds)
          this.append(next.workspaceId, "memory.namespace.assigned", "Memory namespaces assigned", {
            ...payload(next),
            namespaceIds: next.namespaceIds,
            primaryNamespaceId: next.primaryNamespaceId ?? null,
          });
      });
    } catch (error) {
  ...
  createCompaction(input: CreateCompactionInput): MemoryCompactionDto {
    return this.dependencies.db.transaction(() => {
      ...
      this.append(input.workspaceId, "memory.compacted", "Memory compacted", {
        ...
        sourceCount: compaction.sources.length,
      });
      return compaction;
    })();
  }
  ...
  private append(workspaceId: string, type: string, title: string, eventPayload: Record<string, unknown>): void {
    this.dependencies.events.appendInCurrentTransaction(
      createEvent({ workspaceId, type, actor: "system", title, payload: eventPayload, privacy: { labels: ["local"] } }),
    );
  }

  private mapError(error: unknown): Error {
    if (error instanceof MemoryConflictError)
      return new MemoryServiceError("conflict", {
...
function payload(memory: MemoryDto): Record<string, unknown> {
  return { memoryId: memory.id, version: memory.version };
}
function hashFromDb(db: SqliteDatabase, id: string): string {
  return (
    db.prepare<{ id: string }, { content_hash: string }>("SELECT content_hash FROM memories WHERE id = @id").get({ id })
      ?.content_hash ?? ""
  );
}
