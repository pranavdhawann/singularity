...
describe("MemoryService", () => {
  it("writes lifecycle events and invalidates stale derived data transactionally", () => {
    const ctx = setup();
    try {
      const memory = ctx.service.create({
        workspaceId: "w_1",
        type: "decision",
        statement: "Use a DB",
        confidence: 0.8,
      ...
      expect(ctx.events.list({ workspaceId: "w_1", order: "asc" }).map((event) => event.type)).toEqual(
        expect.arrayContaining([
          ...
          "memory.approved",
          "memory.pinned",
          "memory.namespace.assigned",
        ]),
      );
      expect(ctx.events.list({ workspaceId: "w_1" })).toHaveLength(6);
      expect(
        ctx.embeddings.listForSources({
          workspaceId: "w_1",
          adapter: "ollama",
          model: "embed",
          sources: [{ kind: "memory", id: memory.id, contentHash: oldHash }],
      ...
      expect(ctx.events.list({ workspaceId: "w_1" }).map((event) => event.type)).toEqual(
        expect.arrayContaining(["memory.outdated", "memory.deleted"]),
      );
    } finally {
      ctx.db.close();
    }
  ...
  it("creates source-linked compactions and maps version conflicts", () => {
    ...
    try {
      ...
      const compaction = ctx.service.createCompaction({
        workspaceId: "w_1",
      ...
      expect(() => ctx.service.mutate(memory.id, { expectedVersion: 99, pinned: true, reason: "stale" })).toThrow(
        MemoryServiceError,
      );
      try {
        ctx.service.mutate(memory.id, { expectedVersion: 99, pinned: true, reason: "stale" });
      } catch (error) {
        expect(error).toMatchObject({ code: "conflict", details: { expectedVersion: 99, actualVersion: 1 } });
      }
    } finally {
      ctx.db.close();
    }
