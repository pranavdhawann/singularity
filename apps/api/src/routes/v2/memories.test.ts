...
describe("V2 memory routes", () => {
  it("creates, lists, revises, outdates, and deletes memory with strict protection", async () => {
    ...
    const revisedResponse = await server.inject({
      method: "PATCH",
      url: `/api/v2/memories/${created.id}`,
      headers,
      payload: {
        expectedVersion: 1,
    ...
    expect(stale.json()).toEqual({
      error: expect.objectContaining({ code: "conflict", details: { expectedVersion: 1, actualVersion: 2 } }),
    });
    const list = await server.inject({
      method: "GET",
      url: "/api/v2/memories?workspaceId=w_1&reviewState=approved&limit=1",
    });
