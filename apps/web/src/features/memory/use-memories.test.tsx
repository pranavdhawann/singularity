...
const memory: MemoryDto = { id: "mem_1", workspaceId: "w_1", type: "fact", statement: "Use SQLite",
  confidence: 1, reviewState: "proposed", pinned: false, version: 1, namespaceIds: [], sourceIds: [],
...
describe("useMemories", () => {
  it("loads workspace state and refreshes after optimistic mutation", async () => {
    ...
    const api = { listMemories, listNamespaces: vi.fn(async () => ({ namespaces: [] })),
      updateMemory: vi.fn(async () => ({ ...memory, pinned: true, version: 2 })),
      getMemory: vi.fn(async () => memory), listMemoryRevisions: vi.fn(async () => ({ revisions: [] })) } as unknown as FutureApi;
    const { result } = renderHook(() => useMemories(api, "w_1"));
    await act(async () => { await Promise.resolve(); });
    expect(result.current.memories).toEqual([memory]);
    await act(async () => { await result.current.update(memory.id, { expectedVersion: 1, pinned: true, reason: "user_edit" }); });
