  ...
  const loadSelected = useCallback(
    async (id: string) => {
      try {
        ...
        setActionError(cause instanceof Error ? cause.message : "Memory could not be loaded");
      }
    },
    [api],
  );
    ...
    <>
      <section className="main-column memory-workspace" aria-label="Memory workspace">
        <section className="memory-toolbar">
          <div>
            ...
            <p>Review queue: {state.memories.filter((memory) => memory.reviewState === "proposed").length}</p>
          </div>
          <label>
            Status filter
            <select
              ...
              onChange={(event) => state.setReviewState(event.target.value)}
            >
        ...
        <div className="namespace-list" aria-label="Memory namespaces">
          <button
            type="button"
            className={!state.namespaceId ? "selected" : ""}
            onClick={() => state.setNamespaceId("")}
          >
            All memory
          </button>
          {state.namespaces.map((namespace) => (
            <button
              type="button"
              key={namespace.id}
              className={state.namespaceId === namespace.id ? "selected" : ""}
              onClick={() => state.setNamespaceId(namespace.id)}
            >
              {namespace.name}
            </button>
          ))}
        ...
        <div className="memory-create-row">
          <form
            onSubmit={(event) => {
              ...
              void run(async () => {
                const created = await state.create(newMemory.trim());
                setNewMemory("");
          ...
          >
            <label>
              New memory
              <input aria-label="New memory" value={newMemory} onChange={(event) => setNewMemory(event.target.value)} />
            </label>
          ...
          <form
            onSubmit={(event) => {
              ...
              void run(async () => {
                await state.createNamespace(newNamespace.trim());
                setNewNamespace("");
              });
            }}
          >
        ...
        <div className="memory-list">
          ...
          {state.memories.map((memory) => (
            ...
            >
              ...
              <small>
                Version {memory.version} · confidence {memory.confidence}
              </small>
            </button>
          ))}
        </div>
      ...
      <MemoryInspector
        ...
        onSave={async (input) => {
          if (!selected) return;
          await run(async () => {
            const next = await state.update(selected.id, input);
            setSelected(next);
          });
        }}
        onDelete={async () => {
          if (!selected) return;
          await run(async () => {
            await state.remove(selected.id, selected.version);
            setSelectedId(undefined);
            setSelected(undefined);
            setRevisions([]);
          });
        ...
        onCompact={async (summary) => {
          if (!selected?.contentHash) return;
          await run(async () => {
            await api.createCompaction({
              workspaceId,
              summary,
