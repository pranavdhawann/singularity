...
function api(): FutureApi {
  return {
    listMemories: vi.fn(async () => ({ items: [memory] })),
    ...
    listMemoryRevisions: vi.fn(async () => ({
      revisions: [
        {
          id: "rev_1",
          memoryId: memory.id,
          version: 1,
          previous: {},
          next: { statement: memory.statement },
          reason: "created",
          createdAt: memory.createdAt,
        },
      ],
    ...
    createCompaction: vi.fn(async () => ({
      id: "cmp_1",
      workspaceId: "w_1",
      summary: "Database decision",
      contentHash: "cmp_hash",
      sources: [{ kind: "memory", id: "mem_1", contentHash: "mem_hash" }],
      invalidatedAt: null,
      createdAt: memory.createdAt,
    })),
...
describe("MemoryWorkspace", () => {
  it("filters, edits, assigns, and shows revisions", async () => {
    ...
    await waitFor(() =>
      expect(client.createCompaction).toHaveBeenCalledWith(
        expect.objectContaining({
          workspaceId: "w_1",
          summary: "Database decision",
        }),
