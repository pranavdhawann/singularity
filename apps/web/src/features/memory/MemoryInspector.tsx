...
export function MemoryInspector({
  ...
  useEffect(() => {
    ...
    setReviewState(memory.reviewState);
  ...
  if (!memory)
    return (
      <aside className="inspector memory-inspector" aria-label="Memory inspector">
        <p>Select a memory to inspect it.</p>
      </aside>
    );
  return (
    <aside className="inspector memory-inspector" aria-label="Memory inspector">
      ...
      <form
        onSubmit={(event) => {
          ...
          void onSave({
            ...
            reviewState,
            pinned,
            namespaceIds: primaryNamespaceId ? [primaryNamespaceId] : [],
            primaryNamespaceId: primaryNamespaceId || null,
            reason: "user_edit",
          });
        }}
      >
        ...
        <label>
          ...
          <select
            ...
            value={reviewState}
            onChange={(event) => setReviewState(event.target.value as MemoryDto["reviewState"])}
          >
        ...
        <label>
          ...
          <select
            ...
            onChange={(event) => setPrimaryNamespaceId(event.target.value)}
          >
        ...
        <div className="memory-actions">
          ...
          <button
            type="button"
            onClick={() => {
              if (window.confirm("Delete this memory?")) void onDelete();
            }}
          >
            Delete memory
          </button>
      ...
      <div className="inspector-group">
        ...
        <p>{memory.sourceIds.length} linked sources</p>
        {memory.sourceIds.map((sourceId) => (
          <code key={sourceId}>{sourceId}</code>
        ))}
        <p>Confidence {memory.confidence}</p>
      </div>
      <form
        className="compaction-form"
        onSubmit={(event) => {
          event.preventDefault();
      ...
      >
        <label>
          ...
          <textarea
            ...
            onChange={(event) => setCompactionSummary(event.target.value)}
          />
        </label>
        <button type="submit" disabled={!memory.contentHash}>
          Create compaction
        </button>
      </form>
