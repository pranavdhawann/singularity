      ...
      {pack ? (
        <>
          <div className="inspector-group">
            ...
            {pack.retrieval?.mode === "lexical" ? (
              <p className="retrieval-fallback">
              ...
              <article className="context-source-card" key={`${item.source.kind}:${item.source.id}`}>
                ...
                {item.retrieval ? (
                  <dl className="retrieval-details">
                    <div>
                      <dt>Final</dt>
                      <dd>Final score {item.retrieval.finalScore.toFixed(3)}</dd>
                    </div>
                    <div>
                      <dt>Channels</dt>
                      <dd>
                        {item.retrieval.lexicalScore !== undefined
                          ? `Lexical ${item.retrieval.lexicalScore.toFixed(3)}`
                          : "No lexical score"}
                        {item.retrieval.vectorScore !== undefined
                          ? ` · Vector ${item.retrieval.vectorScore.toFixed(3)}`
                          : ""}
                      </dd>
                    </div>
                    <div>
                      <dt>Reasons</dt>
                      <dd>{item.retrieval.reasons.map((reason) => reason.replaceAll("_", " ")).join(", ")}</dd>
                    </div>
                  </dl>
                ) : null}
                {item.compactionSources?.map((source) => (
                  <small className="compaction-source" key={`${source.kind}:${source.id}`}>
                    Compacted from {source.kind.replaceAll("_", " ")} {source.id}
                  </small>
                ))}
                {item.source.range ? (
                  <small>
                    Characters {item.source.range.start}-{item.source.range.end}
                  </small>
                ) : null}
              </article>
            ))}
